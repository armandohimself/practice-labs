...
public class CharComparison {
    ...
    public int compare(char[] a, char[] b){
        ...
        for(int i = 0; i < a.length; i++) {
            String compareA = String.valueOf(a[i]);
            String compareB = String.valueOf(b[i]);

            if(compareA.compareTo(compareB) == -1) return -1;
            if(compareA.compareTo(compareB) > 0) return 1;
        }
